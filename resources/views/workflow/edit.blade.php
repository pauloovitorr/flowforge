<x-layouts.sistema>
    ...
    <div class="w-full grid grid-cols-1">
        <x-sistema.page-presentation icon="network" page="Edição de Workflow">
            ...
            <form action="{{ route('workflow.update', [$workflow->id]) }}" method="post" id="form-workflow" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input type="text" id="w-name" name="name" value="{{ $workflow->name }}"
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <select id="project_id" name="project_id"
                            ...
                            @foreach($projects as $project)
                                <option value="{{ $project->id }}" {{ $project->id == $workflow->project_id ? 'selected' : '' }}>
                                    {{ $project->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col gap-1">
                        <label for="w-event" class="text-sm font-medium text-zinc-700">Evento de Gatilho (Trigger) <span
                                class="text-red-600">*</span></label>
                        <input type="text" id="w-event" name="trigger_event" value="{{ $workflow->trigger_event }}"
                            class="w-full p-2.5 border border-zinc-300 rounded-lg outline-none focus:border-cyan-500 transition-all"
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <select id="w-status" name="status"
                            class="w-full p-2.5 border border-zinc-300 rounded-lg outline-none focus:border-cyan-500 transition-all"
                            required>
                            <option value="active" {{ $workflow->status == 'active' ? 'selected' : '' }}>Ativo</option>
                            <option value="inactive" {{ $workflow->status == 'inactive' ? 'selected' : '' }}>Inativo
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <textarea id="w-desc" name="description" rows="2"
                        ...
                        maxlength="255">{{ $workflow->description }}</textarea>
                </div>

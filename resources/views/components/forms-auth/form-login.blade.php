    ...
    <form action="{{ route('login') }}" method="POST" class="space-y-4">
        ...
        <div class="flex items-center justify-between">
            <label class="inline-flex items-center text-sm text-gray-600">
                <input type="checkbox" name="remember" class="rounded border-gray-300 text-cyan-500 
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}"
                    class="text-sm text-cyan-500 hover:text-cyan-700 underline duration-300 ease-in-out">
                    Esqueceu a senha?
                </a>
            @endif
        </div>
        ...
        <div class="mt-6 flex justify-center">
            <span class="text-center text-sm">
                Não tem conta? 
                <a href="{{ route('register') }}" class=" text-cyan-500 hover:text-cyan-700 underline duration-300 ease-in-out">Cadastre-se!</a>
            </span>
        </div>
